...
export async function middleware(request: NextRequest) {
  ...
  // Public routes: rate limiting + request ID
  const baseResponse = NextResponse.next({ request: { headers: forwardHeaders } });
  ...
  if (publicRoute) {
    ...
    if (rateLimit.limited) {
      ...
      const response = NextResponse.json(
        { error: true, code: 'RATE_LIMIT_EXCEEDED', message: 'Too many requests. Please try again in a minute.' },
        {
          status: 429,
          headers: {
